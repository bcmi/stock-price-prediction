...
from keras.layers import SimpleRNN, LSTM, Dense, Activation, Dropout, Flatten
...
def get_model(train_data,lstmFirstLayer,lstmSecondLayer,lstmThirdLayer):
    ...
    #rnn+lstm
    model = Sequential()
    model.add(SimpleRNN(lstmFirstLayer, input_shape=(trainX.shape[1], trainX.shape[2]),return_sequences=True))
    ...
    history = model.fit(trainX,trainY,batch_size=125,epochs=30,validation_data=(testX,testY),verbose=1,shuffle=True)
...
def output_result(result):
    stu = ['caseid','midprice']
    out = open('result8.csv','w', newline='')
