...
# load模型预测时记得注释掉!!!---------------
model.fit(X_train, Y_train, epochs=4, batch_size=64, validation_data=(X_val, Y_val), callbacks=[callback], shuffle=True)
...
# for i in range(1000):
#     if (res[i][0] > 1e-4):
#         res[i][0] = base_mid_price[i] + delta[i]
#     elif (res[i][0] < -1e-4):
#         res[i][0] = base_mid_price[i] - delta[i]
...
with open('res.csv', 'w', encoding='utf8', newline='') as fout:
    ...
    for i in range(142, len(res)):
        ...
        writer.writerow({'caseid':str(i+1),'midprice':float(res[i][0] / 1000.0 + base_mid_price[i])})
